...
<!-- Interactive Landing Page Calendar Section -->
<section class="py-5 bg-white border-top border-bottom" id="calendar">
    <div class="container py-4">
        <div class="row align-items-end mb-4">
            <div class="col-lg-7">
                <span class="badge bg-success bg-opacity-10 text-success rounded-pill px-3 py-2 fw-semibold mb-2">
                    <i class="fa-solid fa-calendar-days me-1"></i> Live Court Availability
                </span>
                <h2 class="font-heading fw-bold display-6 text-dark mb-1">Interactive Facility Schedule Calendar</h2>
                <p class="text-secondary mb-0">Inspect real-time reservations across all venues or filter by your favorite facility before booking</p>
            </div>
            <div class="col-lg-5 mt-3 mt-lg-0">
                <div class="d-flex flex-wrap gap-2 justify-content-lg-end align-items-center">
                    <div class="me-2">
                        <select id="facilityFilter" class="form-select rounded-pill px-3 py-2 font-heading small shadow-sm border-secondary-subtle">
                            <option value="">All Facilities & Venues</option>
                            @foreach($facilities as $fac)
                                <option value="{{ $fac->id }}">{{ $fac->name }} ({{ ucfirst(str_replace('_', ' ', $fac->sport_type)) }})</option>
                            @endforeach
                        </select>
                    </div>
                    <a href="{{ route('customer.bookings.wizard') }}" class="btn btn-accent rounded-pill px-4 py-2 font-heading fw-semibold fs-6">
                        Book a Court <i class="fa-solid fa-plus ms-1"></i>
                    </a>
                </div>
            </div>
        </div>

        <!-- Sport Badges / Legend Strip -->
        <div class="d-flex flex-wrap align-items-center gap-3 mb-4 p-3 bg-light rounded-4 border border-secondary border-opacity-10">
            <span class="small fw-bold text-uppercase text-muted me-2" style="font-size: 0.75rem; letter-spacing: 0.5px;">Sport Legend:</span>
            <span class="badge rounded-pill text-white px-3 py-2" style="background-color: #10b981;"><i class="fa-solid fa-feather-pointed me-1"></i> Badminton</span>
            <span class="badge rounded-pill text-white px-3 py-2" style="background-color: #f97316;"><i class="fa-solid fa-basketball me-1"></i> Basketball</span>
            <span class="badge rounded-pill text-white px-3 py-2" style="background-color: #3b82f6;"><i class="fa-solid fa-table-tennis-paddle-ball me-1"></i> Pickleball</span>
            <span class="badge rounded-pill text-white px-3 py-2" style="background-color: #eab308;"><i class="fa-solid fa-baseball me-1"></i> Tennis</span>
            <span class="badge rounded-pill text-white px-3 py-2" style="background-color: #06b6d4;"><i class="fa-solid fa-futbol me-1"></i> Futsal</span>
            <span class="ms-auto small text-muted d-none d-md-inline"><i class="fa-solid fa-circle-info text-primary me-1"></i> Click any event to inspect details or book an open slot</span>
        </div>

        <!-- Calendar Container -->
        <div class="glass-card p-3 p-md-4 shadow-sm border rounded-4 bg-white overflow-hidden">
            <div id="homePublicCalendar" style="min-height: 600px;"></div>
        </div>
    </div>
</section>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const calendarEl = document.getElementById('homePublicCalendar');
        const facilityFilter = document.getElementById('facilityFilter');
        if (!calendarEl) return;

        function getEventsUrl() {
            let url = "{{ route('calendar.public-events') }}";
            const facilityId = facilityFilter ? facilityFilter.value : '';
            if (facilityId) {
                url += '?facility_id=' + encodeURIComponent(facilityId);
            }
            return url;
        }
        ...
        const calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            eventClick: function(info) {
                ...
                const facId = props.facility_id || 1;

                Swal.fire({
                    title: info.event.title,
                    html: `
                        <div class="text-start p-2">
                            <p class="mb-1"><strong>Venue:</strong> ${props.facility_name || 'Sports Center'}</p>
                            <p class="mb-1"><strong>Court:</strong> ${props.court_name || 'Court'}</p>
                            <p class="mb-1"><strong>Sport:</strong> ${props.sport || 'Sports'}</p>
                            <p class="mb-1"><strong>Status:</strong> <span class="badge bg-secondary">${props.status || 'Reserved'}</span></p>
                            <p class="mb-0 text-muted small">Time Slot: ${info.event.start.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})} - ${info.event.end ? info.event.end.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : 'End'}</p>
                        </div>
                    `,
                    icon: 'info',
                    showCancelButton: true,
                    confirmButtonText: '<i class="fa-solid fa-calendar-check me-1"></i> Book Open Slot',
                    ...
                    if (result.isConfirmed) {
                        window.location.href = "{{ route('customer.bookings.wizard') }}?facility_id=" + facId;
                    }
                });
            },
            dateClick: function(info) {
                const selectedFacility = facilityFilter ? facilityFilter.value : '';
                let redirectUrl = "{{ route('customer.bookings.wizard') }}";
                if (selectedFacility) {
                    redirectUrl += "?facility_id=" + selectedFacility;
                }
